...
Includes semi-transparent HUD, depth progress bar, and session CSV logging.
"""
import cv2
...
# --- INITIALIZE CSV SESSION LOGGING (Folder-Locked) ---
csv_file = os.path.join(os.path.dirname(__file__), "session_data.csv")
with open(csv_file, mode='w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["rep_number", "timestamp", "knee_angle", "time_since_last_rep"])

session_start_time = time.time()
last_rep_time = session_start_time
    ...
    if detection_result.pose_landmarks:
        ...
        try:
            ...
            if angle < 100 and stage == "up":
                ...
                # Dynamic rep timing & CSV writing
                now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                current_time = time.time()
                time_since_last = round(current_time - last_rep_time, 2)
                last_rep_time = current_time
                
                with open(csv_file, mode='a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([counter, now, angle, time_since_last])
                
                print(f"Rep counted! Total Squats: {counter} | Pace: {time_since_last}s")
